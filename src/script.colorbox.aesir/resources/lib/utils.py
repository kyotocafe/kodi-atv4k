...
def randomartc(img):
    return RandomartCol_Image(img, 1)
def randomarte(img):
    return RandomartCol_Image(img, 1)
def perlin(img):
...
def RandomartEqn_Image(rname="teste", blurr=10):
    md5 = hashlib.md5(rname).hexdigest()
    filename = md5 + str(time.mktime(time.gmtime())) + ".png"
    ...
    for i in range(1):
        ...
        imgfilter = MyGaussianBlur(radius=blurr)
        img = img.filter(imgfilter)
    img.save(targetfile)
    return targetfile
def RandomartCol_Image(rname="testc", blurr=10):
    md5 = hashlib.md5(rname).hexdigest()
    filename = md5 + str(time.mktime(time.gmtime())) + ".png"
    targetfile = os.path.join(ADDON_DATA_PATH, filename)
    random.seed()
...
def Perlin_Image(img, imageSize=64):
    ...
    for i in range(0, imageSize):
       for j in range(0, imageSize):
            ...
            pixels[i, j] = c
    return img
def RandomartDel_Image(rname, blurr=15, gname='sunshine', ln_color=line_color):
    md5 = hashlib.md5(rname).hexdigest()
    filename = md5 + str(time.mktime(time.gmtime())) + ".png"
    targetfile = os.path.join(ADDON_DATA_PATH, filename)
    size = (1920/quality, 1080/quality)
    if equilateral_tris:
...
def End_Time():
    try:
        hours, minutes = xbmc.getInfoLabel('ListItem.EndTime').strip().split(':')
        HOME.setProperty("EndTime_Hours", hours)
        HOME.setProperty("EndTime_Minutes", minutes)
    except:
        return
