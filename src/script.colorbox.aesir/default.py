...
class ColorBoxMain:
    def __init__(self):
        ...
        while self.daemon and not monitor.abortRequested():
            ...
            self.manual_set_NINE = xbmc.getInfoLabel("Control.GetLabel(7979)")
            if (self.manual_set_NINE != '' and self.manual_set_NINE != self.prefix_prev_NINE) or HOME.getProperty("NINE_daemon_fire"):
                ...
                try:
                    ...
                    for larg in self.manual_set_NINE.strip().split('|'):
                        ...
                        if self.var != "" and self.set != "":
                            ColorBox_settings_map[self.var](self.set)
                        #elif self.info == 'randomartc':
                        #    HOME.setProperty(self.prefix + "ImageFilterNINE", utils.RandomartCol_Image(self.id))
                        #elif self.info == 'randomarte':
                        #    HOME.setProperty(self.prefix + "ImageFilterNINE", utils.RandomartEqn_Image(self.id))
                        #elif self.info == 'randomartd':
                        #    HOME.setProperty(self.prefix + "ImageFilterNINE", utils.RandomartDel_Image(self.id))
                        elif self.info != "" and self.id != "":
...
if __name__ == "__main__":
    ...
    if infom == 'randomcolor':
        ...
        us1.start()
    elif infom == 'randomartc':
        HOME.setProperty(prefixm + "ImageFilter", utils.RandomartCol_Image(idm, int(varm)))
    elif infom != "" and idm != "":
